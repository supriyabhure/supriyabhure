	...
	function __construct()
	{
		...
		switch($path)
		{
				...
				if(isset($_REQUEST['submit']))
			{
				...
				$where=array("User_name"=>$Username,"Password"=>$enc_pass);
		...
		if(isset($_REQUEST['btn_Employee_id']))
		{
			...
			include_once('view car_type.php');
			break;
			...
			case '/making_booking':
				if(isset($_REQUEST['submit']))
				{
					...
					if($run)
					{
						...
						window.location='making_booking';
						</script>";
					}
					else
					{
			...
			include_once('making_booking.php');
			break;
			...
			case '/add_review':
				if(isset($_REQUEST['submit']))
				{
					...
					$run=$this->insert('reviews',$arr);
					if($run)
					{
						echo "<script>
						
						alert('review success');
						window.location='add_review';
						</script>";
					}
					else
					{
			...
			case '/edit_booking':
				...
				if(isset($_REQUEST['update']))
				{
					...
					if($_FILES['Img']['size']>0)
					{
						...
						$res=$this->update_where('booking',$arr,$where);
						if($res)
						{
							unlink('upload/booking/'.$old_Img);
							echo "<script>
							alert('Update success');
							window.location='profile';
							</script>";
